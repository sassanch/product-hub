...
const INITIATIVES = `query RoadmapInitiatives { initiatives(first: 50, orderBy: updatedAt) { nodes { id name summary status health targetDate owner { name } initiativeUpdates(first: 1, orderBy: createdAt) { nodes { body createdAt } } } } }`;
const PROJECTS = `query RoadmapProjects { projects(first: 100, orderBy: updatedAt, includeArchived: false) { nodes { id name summary health startDate targetDate status { name type } lead { name } teams(first: 20) { nodes { name } } initiatives(first: 10) { nodes { id } } projectMilestones(first: 20) { nodes { id name description targetDate } } projectUpdates(first: 1, orderBy: createdAt) { nodes { body createdAt } } } } }`;
