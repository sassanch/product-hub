...
function mapProjects(ranges: ValueRange[]): Project[] {
  const [identity, summary, status, priority, milestones, lead, dates, portfolio, initiativeIds] = ranges;
  const rows = Math.max(...ranges.map((range) => range.values?.length ?? 0));
  ...
  for (let row = 0; row < rows; row += 1) {
    ...
    projects.push({
      ...
      priority: cell(priority, row) || null,
      health,
...
async function fetchSheetsSnapshot() {
  ...
  try {
    ...
    const [initiativeRanges, projectRanges] = await Promise.all([
      ...
      readRanges(projectsId, ["Projects!A2:B1000", "Projects!D2:D1000", "Projects!F2:F1000", "Projects!G2:G1000", "Projects!H2:H1000", "Projects!J2:J1000", "Projects!L2:O1000", "Projects!V2:Z1000", "Projects!AF2:AF1000"], accessToken),
    ]);
