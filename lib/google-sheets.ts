...
export function mapProjects(ranges: ValueRange[]): Project[] {
  const [identity, summary, descriptionMarkdown, status, priority, milestones, lead, dates, portfolio, initiativeIds] = ranges;
  const rows = Math.max(...ranges.map((range) => range.values?.length ?? 0));
  ...
  for (let row = 0; row < rows; row += 1) {
    ...
    projects.push({
      ...
      descriptionMarkdown: cell(descriptionMarkdown, row) || null,
      status: cell(status, row),
